...
- **Open, not yet fixed** (real functional/UX bugs found on-device, not yet triaged into a phase): Logger logged sets don't persist; no way to delete a logged set (wants swipe-to-delete); the felt-rating picker's actual save behavior needs re-verification; Settings' diet/weight/birth-date/location fields don't persist on Save; HealthKit sync doesn't visibly do anything; the Goals section renders as two visually disjoint cards (layout bug); the dropdown-add picker (Goals/Activities) doesn't function; Pains text-entry UX needs a rework; Home's "Swap activity" sheet is still just the documented not-available-yet shell. General aesthetic pass also still pending (Sohan's words: "ignore for now").

**Outstanding, needs Sohan:** no Anthropic API key configured yet (engine runs safely without Claude's smarter picks until then — meaning every recommendation generated so far has been the fallback-template path, not Claude); the on-demand exercise-swap backend is a plain function, not yet wrapped in a callable Edge Function; the rest of the mobile nav/HealthKit/Trends-charts on-device walkthrough (beyond what the 2026-06-26 pass already covered for Home/Settings/Trends) is still owed.

**Next up:** work through the 2026-06-26 on-device bug list above, starting with Logger (logging is the app's core daily-use loop) and Settings (every field needs to actually persist before the screen is usable).
