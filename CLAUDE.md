...
## Status
Phase 0/1 (scaffolding + schema) and Phase 2 (weight-tuning prototype) are both complete — see `docs/superpowers/specs/2026-06-20-bulletproof-architecture-design.md` for the full phased roadmap. Full Oura history (2024-08-21 to present) is live in Supabase's `recovery` + `activity` tables; reconstructed session history (665 days, combining the Strong export with Oura's auto-detected workouts plus manual gap-filling) is live in `sessions`. A first-pass scoring engine (`prototyping/weight-tuning/scoring.py`) runs against that real data in `prototyping/weight-tuning/weight_tuning.py`, currently matching real history on only ~3% of days — expected for untuned weights against partly-synthetic ground truth (see that notebook's "Known ground-truth limitations" cell). **Next:** iterate on the weights in `scoring.py` until the recommendations feel right, then move to Phase 3 (productionize into `engine/`).
