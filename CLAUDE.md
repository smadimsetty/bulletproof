...
**Still open, deferred, not urgent:** General aesthetic/visual-polish pass explicitly deferred by Sohan until function is right.

**Outstanding, needs Sohan:** two Supabase Edge Functions are written and committed but **not deployed yet** — `trigger-daily-engine` (2026-06-26) and the new `trigger-swap-activity` (2026-07-05) both need a fine-grained GitHub PAT (`actions:write` on `smadimsetty/bulletproof` only) plus `npx supabase secrets set GITHUB_PAT=...` and `npx supabase functions deploy <name>` for each; confirmed via `npx supabase functions list` currently returning empty. Until deployed, both the on-demand daily trigger and the new swap feature fail their dispatch calls gracefully and fall back to their safe states (cron-only recommendations; an inert "couldn't start the swap" message). No Anthropic API key configured yet (engine runs safely without Claude's smarter picks until then). The rest of the mobile nav/HealthKit/Trends-charts on-device walkthrough (beyond what the 2026-06-26 passes already covered for Home/Settings/Trends/Logger) is still owed — none of the 2026-06-26 Logger/Settings fixes, the on-demand trigger flow, nor the new swap feature have been seen rendering on a real device yet. Also unrelated to code: Sohan's Oura ring/app hasn't synced in about a week and no session has been logged in the mobile app in about 10 days.
