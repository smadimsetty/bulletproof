...
**Outstanding, needs Sohan:** the new `trigger-daily-engine` Edge Function is written and committed but **not deployed yet** — needs a fine-grained GitHub PAT (`actions:write` on `smadimsetty/bulletproof` only) plus `npx supabase secrets set GITHUB_PAT=...` and `npx supabase functions deploy trigger-daily-engine`; until then the on-demand trigger fails gracefully and the system behaves exactly as before (cron-only). No Anthropic API key configured yet (engine runs safely without Claude's smarter picks until then — meaning every recommendation generated so far has been the fallback-template path, not Claude); the on-demand exercise-swap backend is a plain function, not yet wrapped in a callable Edge Function; the rest of the mobile nav/HealthKit/Trends-charts on-device walkthrough (beyond what the 2026-06-26 passes already covered for Home/Settings/Trends/Logger) is still owed — specifically, none of the 2026-06-26 Logger/Settings fixes, nor the new on-demand trigger flow, have been seen rendering on a real device yet either.

**Next up:** deploying the `trigger-daily-engine` Edge Function (needs Sohan's GitHub PAT) and the on-device walkthrough that verifies it actually triggers and lands within ~a minute; the broader on-device walkthrough that verifies all the 2026-06-26 fixes actually work as intended; the aesthetic/visual-polish pass once function is confirmed solid.
